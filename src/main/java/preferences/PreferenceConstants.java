package preferences;

public class PreferenceConstants {
	public static final String P_TABLE = "P_TABLE";
	public static final String P_POPUP_WIDTH = "P_POPUP_WIDTH";
	public static final String P_POPUP_HEIGHT = "P_POPUP_HEIGHT";
	public static final String P_POPUP_SIZE = "P_POPUP_SIZE";
	public static final String P_SUGGEST = "P_SUGGEST";
	public static final String P_TEMP_FILES = "P_TEMP_FILES";
	public static final String P_FONT_SIZE = "P_FONT_SIZE";
	public static final String P_LOG_ERRORS = "P_LOG_ERRORS";
}
